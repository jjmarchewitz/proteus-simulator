...
#define DotClk_Polarity \
  1  // 0:Data latch in rising edge; 1:Data latch in falling edge
...
#define TFT_Type \
  0  // 0 or 1:TFT mode; 2:Serial RGB mode; 3:Serial RGB+dummy mode
...
#define Panel_Mode1                                                            \
  (Panel_Data_Width_Bit << 5) | (1 << 4) | (1 << 3) | (DotClk_Polarity << 2) | \
      (Hsync_Polarity << 1) | (Vsync_Polarity)
#define Panel_Mode2                                                        \
  (LCD_Panel_Mode << 7) | (TFT_Type << 5) | (LCD_Data_Pin_Strength << 2) | \
      (LCD_Control_Pin_Strength << 0)
#define Panel_Mode3 G

// Horizontal Cycle = Horizontal Pulse Width + Horizontal Back Porch +
// Horizontal Display Period + Horizontal Front Porch Horizontal Start Position
// = Horizontal Pulse Width + Horizontal Back Porch
...
// Vertical Cycle = Vertical Pulse Width + Vertical Back Porch + Vertical
// Display Period + Vertical Front Porch Vertical Start Position = Vertical
// Pulse Width + Vertical Back Porch
...
#define LCD_HORI_CYCLE                                      \
  (LCD_HORI_PULSE_WIDTH + LCD_HORI_BACK_PORCH + LCD_WIDTH + \
   LCD_HORI_FRONT_PORCH)
#define LCD_VERT_CYCLE                                       \
  (LCD_VERT_PULSE_WIDTH + LCD_VERT_BACK_PORCH + LCD_HEIGHT + \
...
void FEHLCD::_Initialize() {
  ...
  WriteParameter(
      Pixel_Data_Interface_Format);  // 0x04:18bit,0x03:16bit,0x05:24bit
  ...
  // wpat( 0x00 ); //set page,column,line,RGB adress order;display data latch
  // data,fli horizontal,vertical
  WriteParameter(0x00);
  ...
  WriteIndex(0xBE);
  WriteParameter(0x0E);  //(set PWM frequency) PWM signal frequency = PLL clock
                         /// (256 * PWMF[7:0]) / 256
...
void FEHLCD::PrintImage(int x, int y) {
  ...
  for (int i = 0; i < image_length; i += 2) {
    for (int j = 0; j < image[i]; j++) {
      ...
      //_forecolor =
      //ConvertRGBColorTo16Bit(image[k][0],image[k][1],image[k][2]);
...
unsigned int FEHLCD::ConvertRGBColorTo16Bit(unsigned char r,
                                            unsigned char g,
                                            unsigned char b) {
  unsigned int ru = r >> 2;
  unsigned int gu = g >> 2;
...
void FEHLCD::WriteIndex(unsigned char index) {
  ...
  GPIOC_PDOR &= ~GPIO_PDOR_PDO((1 << 4) | (1 << 5) | (1 << 6) | (1 << 7) |
                               (1 << 12) | (1 << 13));
...
void FEHLCD::SetRegisterColorValues() {
  ...
  backRegisterValues.BVal |=
      GPIO_PDOR_PDO((((c1 & (1 << 0)) ? (1) : (0)) << 9));
  backRegisterValues.BVal |=
      GPIO_PDOR_PDO((((c1 & (1 << 1)) ? (1) : (0)) << 8));
  backRegisterValues.CVal |=
      GPIO_PDOR_PDO((((c1 & (1 << 2)) ? (1) : (0)) << 4));
  backRegisterValues.CVal |=
      GPIO_PDOR_PDO((((c1 & (1 << 3)) ? (1) : (0)) << 5));
  backRegisterValues.CVal |=
      GPIO_PDOR_PDO((((c1 & (1 << 4)) ? (1) : (0)) << 6));
  backRegisterValues.CVal |=
      GPIO_PDOR_PDO((((c1 & (1 << 5)) ? (1) : (0)) << 7));
  backRegisterValues.CVal |=
      GPIO_PDOR_PDO((((c1 & (1 << 6)) ? (1) : (0)) << 12));
  backRegisterValues.CVal |=
      GPIO_PDOR_PDO((((c1 & (1 << 7)) ? (1) : (0)) << 13));

  // dataport2 = sel_color[ i + 1 ];
  //    c = sel_color[ i + 1 ];
  backRegisterValues.CVal |=
      GPIO_PDOR_PDO((((c2 & (1 << 0)) ? (1) : (0)) << 14));
  backRegisterValues.CVal |=
      GPIO_PDOR_PDO((((c2 & (1 << 1)) ? (1) : (0)) << 15));
  backRegisterValues.CVal |=
      GPIO_PDOR_PDO((((c2 & (1 << 2)) ? (1) : (0)) << 16));
  backRegisterValues.CVal |=
      GPIO_PDOR_PDO((((c2 & (1 << 3)) ? (1) : (0)) << 17));
  backRegisterValues.CVal |=
      GPIO_PDOR_PDO((((c2 & (1 << 4)) ? (1) : (0)) << 18));
  backRegisterValues.CVal |=
      GPIO_PDOR_PDO((((c2 & (1 << 5)) ? (1) : (0)) << 19));
  backRegisterValues.DVal |=
      GPIO_PDOR_PDO((((c2 & (1 << 6)) ? (1) : (0)) << 0));
  backRegisterValues.DVal |=
      GPIO_PDOR_PDO((((c2 & (1 << 7)) ? (1) : (0)) << 2));

  // dataport3 = sel_color[ i + 2 ];
  //    c = sel_color[ i + 2 ];
  backRegisterValues.DVal |=
      GPIO_PDOR_PDO((((c3 & (1 << 0)) ? (1) : (0)) << 3));
  backRegisterValues.DVal |=
      GPIO_PDOR_PDO((((c3 & (1 << 1)) ? (1) : (0)) << 4));
  ...
  foreRegisterValues.BVal |=
      GPIO_PDOR_PDO((((c1 & (1 << 0)) ? (1) : (0)) << 9));
  foreRegisterValues.BVal |=
      GPIO_PDOR_PDO((((c1 & (1 << 1)) ? (1) : (0)) << 8));
  foreRegisterValues.CVal |=
      GPIO_PDOR_PDO((((c1 & (1 << 2)) ? (1) : (0)) << 4));
  foreRegisterValues.CVal |=
      GPIO_PDOR_PDO((((c1 & (1 << 3)) ? (1) : (0)) << 5));
  foreRegisterValues.CVal |=
...
void FEHIcon::Icon::SetProperties(char name[20],
                                  int start_x,
                                  int start_y,
                                  int w,
                                  int h,
...
int FEHIcon::Icon::Pressed(float x, float y, int mode) {
  if (x >= x_start && x <= x_end && y >= y_start && y <= y_end) {
    ...
    {
      if (!mode)  // if mode is 0, then alternate selecting and deselecting as
                  // it is pressed again and again; otherwise, the icon does not
                  // select and deselect
      {
        if (!set) {
          Select();
...
void FEHIcon::Icon::ChangeLabelFloat(float val) {
  ...
  /* If the new label is not the same length as the old one, then erase the old
   * one so that it does not show up behind the new one */
 ...
 */
void FEHIcon::DrawIconArray(Icon icon[],
                            int rows,
                            int cols,
                            int top,
                            int bot,
