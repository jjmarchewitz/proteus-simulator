...
class FEHLCD {
 public:
  ...
  typedef enum { North = 0, South, East, West } FEHLCDOrientation;
 ...
 private:
  ...
  unsigned int ConvertRGBColorTo16Bit(unsigned char r,
                                      unsigned char g,
                                      unsigned char b);
...
class Icon {
 ...
 public:
  Icon();
  void SetProperties(char name[20],
                     int start_x,
                     int start_y,
                     int w,
                     int h,
...
/* Function prototype for drawing an array of icons in a rows by cols array with
 * top, bot, left, and right margins from edges of screen, labels for each icon
 * from top left across each row to the bottom right, and color for the
 * rectangle and the text color */
void DrawIconArray(Icon icon[],
                   int rows,
                   int cols,
                   int top,
                   int bot,
