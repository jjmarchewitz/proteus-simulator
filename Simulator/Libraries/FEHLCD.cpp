...
void FEHLCD::Update()
{
    // We used to have students call LCD.Update() regularly in their programs during
    // SDP in the fall semester. This was so that the tigr library could update the screen
    // and the program wouldn't crash (the OS needs constant affirmation that a process is alive).
    // Now, Matthew got LCD.Update() and Dashboard.Update() to be called in their own thread,
    // the same thread where the LCD's "screen" object is declared. Calling LCD.Update() outside of
    // that thread (i.e. inside of the ProteusMain function in main.cpp) will crash the program.
    // This empty function is meant to make student's old code still work, while the real
    // LCD.Update() is now LCD.Update(bool) and is called frequently in the background.
}

void FEHLCD::Update(bool t)
{
    // This function overload exists as a hack-y workaround. See above zero-argument function
    // for an explanation.
    tigrUpdate(screen);
